...
// ... (Keep the rest of your basic top module exactly the same) ...

    // 2. Address Generation (THE FIX IS HERE)
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // FIX: 'start' pushes addr_b to 1 so the ROM fetches Row 1 
            // while the PE array is busy shifting in Row 0!
            if (start || wshift) addr_b <= addr_b + 1; 
        end
    end
    ...
    permutating_weight_memory #(
        ...
        .clk(clk),
        .ren(1'b1),               // FIX: Tie high! Address 0 is now always ready.
